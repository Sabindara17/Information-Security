//Implementation of Playfair Cipher and Railfence Cipher in C program
...
for (k = 0; k < ks; k++) {
if (dicty[key[k] - 97] == 2) {
dicty[key[k] - 97] -= 1; keyT[i][j] = key[k]; j++;
if (j == 5) {
i++;
j = 0; }
} }
...
if (ptrs % 2 != 0) { str[ptrs++] = 'z'; str[ptrs] = '\0';
}
return ptrs; }
...
if (a[0] == a[2]) {
str[i] = keyT[a[0]][mod5(a[1] + 1)]; str[i + 1] = keyT[a[0]][mod5(a[3] + 1)];
}
else if (a[1] == a[3]) {
...
// Plaintext
strcpy(str, "Programming"); printf("Plain text: %s\n", str);
